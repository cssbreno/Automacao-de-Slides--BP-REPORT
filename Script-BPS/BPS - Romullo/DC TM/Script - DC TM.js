...
// Gráfico 1
function createChartTM() {
  const sheet = SpreadsheetApp.openById(SHEETS_ID).getSheetByName(SHEET_DC_TM);
...
function insertChartTM(chart) {
  if (chart) {
    const slides = SlidesApp.openById(SLIDES_ID);
    const slide = slides.getSlides()[slideIndex65];
...
function updateLastThreeChartsTM() {
  const slides = SlidesApp.openById(SLIDES_ID);
  const slide = slides.getSlides()[slideIndex65];
  ...
  if (images.length >= 3) {
    ...
    const sheet =
      SpreadsheetApp.openById(SHEETS_ID).getSheetByName(SHEET_DC_TM);
