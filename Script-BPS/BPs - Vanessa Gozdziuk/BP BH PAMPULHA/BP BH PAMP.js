...
// Gráfico 1
function createChartBH() {
  const sheet =
    SpreadsheetApp.openById(SHEETS_ID).getSheetByName(SHEET_BH_PAMP);
  ...
  insertChartIntoSlideBH(chart);
}

// Gráfico 2
function createChartBH2() {
  const sheet =
    SpreadsheetApp.openById(SHEETS_ID).getSheetByName(SHEET_BH_PAMP);
...
function insertChartIntoSlideBH(chart) {
  if (chart) {
    const slides = SlidesApp.openById(SLIDES_ID);
    const slide = slides.getSlides()[slideIndex24];
...
function updateLastThreeChartsBH() {
  const slides = SlidesApp.openById(SLIDES_ID);
  const slide = slides.getSlides()[slideIndex24];
...
function updateBP_BHPAMPULHA() {
  ...
  try {
    ...
    // Atualizar os gráficos após atualizar as outras informações
    updateLastThreeChartsBH();
