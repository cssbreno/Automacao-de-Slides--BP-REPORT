  // Nome da planilha
  const SHEET_BP_VCONQ = 'BP VITÓRIA DA CONQUISTA';
  const slideIndex49 = 48; // Índice do slide (baseado em 0)

// Gráfico 1
function createChartVCONQ() {
  const sheet = SpreadsheetApp.openById(SHEETS_ID).getSheetByName(SHEET_BP_VCONQ);
  ...
  insertChartVCONQ(chart);
}

// Gráfico 2
function createChartVCONQ2() {
  const sheet = SpreadsheetApp.openById(SHEETS_ID).getSheetByName(SHEET_BP_VCONQ);
...
function insertChartVCONQ(chart) {
  if (chart) { 
    const slides = SlidesApp.openById(SLIDES_ID);
    const slide = slides.getSlides()[slideIndex49];
...
function updateLastThreeChartsVCONQ() {
  const slides = SlidesApp.openById(SLIDES_ID);
  const slide = slides.getSlides()[slideIndex49];
