  // Nome da planilha
  const SHEET_BP_MOEMA = 'SP MOEMA';
  const slideIndex148 = 147; // Índice do slide (baseado em 0)

// Gráfico 1
function createChartBPMOEMA1() {
  const sheet = SpreadsheetApp.openById(SHEETS_ID).getSheetByName(SHEET_BP_MOEMA);
  ...
  insertChartBPMOEMA(chart);
}

// Gráfico 2
function createChartBPMOEMA2() {
  const sheet = SpreadsheetApp.openById(SHEETS_ID).getSheetByName(SHEET_BP_MOEMA);
...
function insertChartBPMOEMA(chart) {
  if (chart) { 
    const slides = SlidesApp.openById(SLIDES_ID);
    const slide = slides.getSlides()[slideIndex148];
...
function updateBP_MOEMA() {
  ...
  try {
    ...
    var sheet = spreadsheet.getSheetByName(SHEET_BP_MOEMA);
    

    // Acessar a apresentação e o slide
    var presentation = SlidesApp.openById(SLIDES_ID);
    var slide148 = presentation.getSlides()[slideIndex148];
    ...
    slide148.getShapes()[147].getText().setText(PN_INPROGRESS_PERCENTAGE_Formatado + "%"); // Em andamento
    slide148.getShapes()[150].getText().setText(PN_SUSPENDED_PERCENTAGE_Formatado + "%"); // Suspensos
    slide148.getShapes()[153].getText().setText(PN_TOTAL); // Total
    slide148.getShapes()[154].getText().setText(DATAMOUNTH); // Período
  ...
  // Dados da Planilha - Colocar a célula da planilha
    ...
    slide148.getShapes()[40].getText().setText(PN2_INPROGRESS_PERCENTAGE_Formatado + "%"); // Em andamento
    slide148.getShapes()[43].getText().setText(PN2_SUSPENDED_PERCENTAGE_Formatado + "%"); // Suspensos
    slide148.getShapes()[173].getText().setText(PN2_TOTAL); // Total
   ...
   // Dados da Planilha - Colocar a célula da planilha
    ...
    slide148.getShapes()[46].getText().setText(PA_INPROGRESS_PERCENTAGE_Formatado + "%"); // Em andamento
    slide148.getShapes()[49].getText().setText(PA_SUSPENDED_PERCENTAGE_Formatado + "%"); // Suspensos
    slide148.getShapes()[176].getText().setText(PA_TOTAL); // Total
    // slide148.getShapes()[##].getText().setText(PA_SUSPENDED_PERCENTAGE + "%"); // Suspensos
    // slide148.getShapes()[##].getText().setText(PA_INPROGRESS_PERCENTAGE + "%"); // Suspensos
    ...
    // ⭐ PROJETOS CONCLUÍDOS 
      ...
      slide148.getShapes()[22].getText().setText(PC_FASE1 + " - Fase 1"); // Projetos Concluídos - 1ª Fase 
      slide148.getShapes()[155].getText().setText(PC_FASE2 + " -  Fase 2"); // Projetos Concluídos - 2ª Fase 
      slide148.getShapes()[20].getText().setText(PC_TOTAL + " TOTAL"); // Projetos Concluídos - Total
    ...
    // ⭐ PEP ⭐ 
      ...
      var PEP_META_NOVOS = sheet.getRange('B51').getValue();
        ...
        slide148.getShapes()[203].getText().setText(PEP_ATINGIDO_NOVOS + "%"); // PEP - Atingido - Projetos Novos
        // slide148.getShapes()[204].getText().setText(PEP_META_NOVOS + "%"); // PEP - Meta - Projetos Novos
      ...
      slide148.getShapes()[200].setHeight(newHeight_PEP_NOVOS); // Ajustar a altura da barra de progresso - Projetos Novos
      ...
      var PEP_META_ANTIGOS = sheet.getRange('B52').getValue();
        ...
        slide148.getShapes()[222].getText().setText(PEP_ATINGIDO_ANTIGOS + "%"); // PEP - Atingido - Projetos Antigos
        // slide148.getShapes()[223].getText().setText(PEP_META_ANTIGOS + "%"); // PEP - Meta - Projetos Antigos
      ...
      slide148.getShapes()[208].setHeight(newHeight_PEP_ANTIGOS); // Ajustar a altura da barra de progresso - Projetos Antigos
    ...
    // ⭐ CONFORMIDADE DE PROJETOS ⭐ 
      ...
      var CONFORMIDADE_COR_FONTE = sheet.getRange('L62').getValue();
        ...
        slide148.getShapes()[143].getText().setText(CONFORMIDADE_ATINGIDO + "%"); // Conformidade - ATINGIDO
        // slide148.getShapes()[144].getText().setText(CONFORMIDADE_META + "%"); // Conformidade - META
        slide148.getShapes()[143].getText().getTextStyle().setForegroundColor(CONFORMIDADE_COR_FONTE); // Conformidade: Meta Atingida - Cor da fonte
      ...
      var BARRA_ENCERRAMENTO = sheet.getRange('K68').getValue();
        ...
        slide148.getShapes()[68].getText().setText(CRONOGRAMA + "%"); // Conformidade - Cronograma
        slide148.getShapes()[66].getText().setText(SINTONIA + "%"); // Conformidade - Sintonia
        slide148.getShapes()[64].getText().setText(APONTAMENTO_HORAS + "%"); // Conformidade - Apontamento de horas
        slide148.getShapes()[85].getText().setText(ESCOPO + "%"); // Conformidade - Escopo
        slide148.getShapes()[83].getText().setText(SIMULACAO + "%"); // Conformidade - Simulação
        slide148.getShapes()[81].getText().setText(KICKOFF + "%"); // Conformidade - Kickoff
        slide148.getShapes()[87].getText().setText(ENCERRAMENTO + "%"); // Conformidade - Encerramento
      ...
      slide148.getShapes()[67].setHeight(newHeight_CRONOGRAMA); // Ajustar a altura da barra de progresso
      slide148.getShapes()[67].getFill().setSolidFill(BARRA_CRONOGRAMA); // Aplicar a cor sólida ao preenchimento
      ...
      slide148.getShapes()[65].setHeight(newHeight_SINTONIA); // Ajustar a altura da barra de progresso
      slide148.getShapes()[65].getFill().setSolidFill(BARRA_SINTONIA); // Aplicar a cor sólida ao preenchimento
      ...
      slide148.getShapes()[63].setHeight(newHeight_APONTAMENTO_HORAS); // Ajustar a altura da barra de progresso
      slide148.getShapes()[63].getFill().setSolidFill(BARRA_APONTAMENTOH); // Aplicar a cor sólida ao preenchimento
      ...
      slide148.getShapes()[84].setHeight(newHeight_ESCOPO); // Ajustar a altura da barra de progresso
      slide148.getShapes()[84].getFill().setSolidFill(BARRA_ESCOPO); // Aplicar a cor sólida ao preenchimento
      ...
      slide148.getShapes()[82].setHeight(newHeight_SIMULACAO); // Ajustar a altura da barra de progresso
      slide148.getShapes()[82].getFill().setSolidFill(BARRA_SIMULACAO); // Aplicar a cor sólida ao preenchimento
      ...
      slide148.getShapes()[80].setHeight(newHeight_KICKOFF); // Ajustar a altura da barra de progresso
      slide148.getShapes()[80].getFill().setSolidFill(BARRA_KICKOFF); // Aplicar a cor sólida ao preenchimento
      ...
      slide148.getShapes()[86].setHeight(newHeight_ENCERRAMENTO); // Ajustar a altura da barra de progresso
      slide148.getShapes()[86].getFill().setSolidFill(BARRA_ENCERRAMENTO); // Aplicar a cor sólida ao preenchimento
    ...
    // ⭐ PROJETOS CONCLUÍDOS - CLUSTER ⭐
      ...
      slide148.getShapes()[37].getText().setText(CLUSTER_500_A); // Cluster <= 500H (ANTIGOS)
      slide148.getShapes()[163].getText().setText(CLUSTER_1000_A); // Cluster <= 1000H (ANTIGOS)
      slide148.getShapes()[166].getText().setText(CLUSTER_1500_A); // Cluster <= 1500H (ANTIGOS)
      slide148.getShapes()[169].getText().setText(CLUSTER_1500MAIS_A); // Cluster > 1500H (ANTIGOS)
      slide148.getShapes()[14].getText().setText(CLUSTER_TOTAL_A); // Cluster TOTAL (ANTIGOS)
      ...
      slide148.getShapes()[50].getText().setText(CLUSTER_500_N); // Cluster <= 500H (NOVOS)
      slide148.getShapes()[164].getText().setText(CLUSTER_1000_N); // Cluster <= 1000H (NOVOS)
      slide148.getShapes()[167].getText().setText(CLUSTER_1500_N); // Cluster <= 1500H (NOVOS)
      slide148.getShapes()[170].getText().setText(CLUSTER_1500MAIS_N); // Cluster > 1500H (NOVOS)
      slide148.getShapes()[19].getText().setText(CLUSTER_TOTAL_N); // Cluster TOTAL (NOVOS)
    ...
    // ⭐ DMP ⭐
      ...
      var DMP_DESCRITIVO = sheet.getRange('B94:F94').getValue();
        ...
        // Ajustar os valores contidos dentro das barras
        slide148.getShapes()[195].getText().setText(DMP_ATINGIDO + "%"); // DMP - Atingido
        // slide148.getShapes()[198].getText().setText(DMP_META + "%"); // DMP - Meta
        // slide148.getShapes()[177].getText().setText(DMP_DESCRITIVO); // DMP - Texto descritivo abaixo da box
      ...
      slide148.getShapes()[196].setHeight(newHeight_DMP); // Ajustar a altura da barra de progresso
      slide148.getShapes()[196].getFill().setSolidFill(BARRA_DMP); // Aplicar a cor sólida ao preenchimento


    // ❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎ SLIDE 149 ❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎❎

      const slideIndex149 = 148; // Índice do slide (baseado em 0)

    // Acessar a apresentação e o slide
    var presentation = SlidesApp.openById(SLIDES_ID);
    var slide149 = presentation.getSlides()[slideIndex149];

        var DATAMOUNTH2 = sheet.getRange('C38').getValue();
      slide149.getShapes()[24].getText().setText(DATAMOUNTH2); // Período
      ...
      var BARRA_SAT_ANTIGOS = sheet.getRange('K109').getValue();
        ...
        slide149.getShapes()[5].getText().setText(SATISFAÇAO_ATINGIDO_PN + "%"); // SATISFAÇÃO - ATINGIDO
        // slide.getShapes()[6].getText().setText(SATISFAÇAO_META_PN + "%"); // SATISFAÇÃO - META
        slide149.getShapes()[5].getText().getTextStyle().setForegroundColor(SATISFAÇAO_COR_FONTE_PN); // Satisfação: Atingido - Cor da fonte

        // Ajustar os valores  - SATISFAÇÃO DE PROJETOS ANTIGOS
        slide149.getShapes()[9].getText().setText(SATISFAÇAO_ATINGIDO_PA + "%"); // SATISFAÇÃO - ATINGIDO
        // slide.getShapes()[10].getText().setText(SATISFAÇAO_META_PA + "%"); // SATISFAÇÃO - META
        slide149.getShapes()[9].getText().getTextStyle().setForegroundColor(SATISFAÇAO_COR_FONTE_PA); // Satisfação: Meta - Cor da fonte
      ...
      slide149.getShapes()[4].setHeight(newHeight_SAT_PN); // Ajustar a altura da barra de progresso
      slide149.getShapes()[4].getFill().setSolidFill(BARRA_SAT_NOVOS); // Aplicar a cor sólida ao preenchimento
      ...
      slide149.getShapes()[8].setHeight(newHeight_SAT_PA); // Ajustar a altura da barra de progresso
      slide149.getShapes()[8].getFill().setSolidFill(BARRA_SAT_ANTIGOS); // Aplicar a cor sólida ao preenchimento
    ...
    // Ofensores - Lista
      ...
      slide149.getShapes()[13].getText().setText(OFENSOR_01); // Item 1 da lista de Ofensores
      slide149.getShapes()[14].getText().setText(OFENSOR_02); // Item 2 da lista de Ofensores
      slide149.getShapes()[16].getText().setText(OFENSOR_03); // Item 3 da lista de Ofensores
      slide149.getShapes()[18].getText().setText(OFENSOR_04); // Item 4 da lista de Ofensores
      slide149.getShapes()[20].getText().setText(OFENSOR_05); // Item 5 da lista de Ofensores
    ...
    // Indice do slide 150
    const slideIndex150 = 149; // Índice do slide (baseado em 0)

    // Acessar a apresentação e o slide
    var presentation = SlidesApp.openById(SLIDES_ID);
    var slide150 = presentation.getSlides()[slideIndex150];

    var DATAMOUNTH2 = sheet.getRange('C38').getValue();
    slide150.getShapes()[37].getText().setText(DATAMOUNTH2); // Período

    // Plano de ação - caixas de texto
      var PLAN_AC_EA = sheet.getRange('B140').getValue();
      var PLAN_AC_ENC = sheet.getRange('B141').getValue();

      slide150.getShapes()[33].getText().setText(PLAN_AC_EA); // Plano de ação - Em Aberto
      slide150.getShapes()[36].getText().setText(PLAN_AC_ENC); // Plano de ação - Encerrados

    // Reclamações - caixa de texto
      var RECL_AC_EA = sheet.getRange('B150').getValue();
      var TOTAL_PORTF = sheet.getRange('B151').getValue();

      slide150.getShapes()[5].getText().setText(RECL_AC_EA); // // Reclamações - Em Aberto
      slide150.getShapes()[8].getText().setText(RECL_AC_EA); // // Reclamações - Em Aberto
      slide150.getShapes()[3].getText().setText(TOTAL_PORTF); // // Reclamações - Total do Portfólio
      ...
      slide150.getShapes()[18].getText().setText(VLR_MMR_Formatado); // MMR - Total
      ...
      slide150.getShapes()[9].getText().setText(CL1); // Clientes / Unidades (1)
      slide150.getShapes()[10].getText().setText(CL2); // Clientes / Unidades (2)
      slide150.getShapes()[12].getText().setText(CL3); // Clientes / Unidades (3)
      slide150.getShapes()[14].getText().setText(CL4); // Clientes / Unidades (4)
      slide150.getShapes()[16].getText().setText(CL5); // Clientes / Unidades (5)
      ...
      slide150.getShapes()[23].getText().setText(CL1_MMR + " mil"); // MMR por Cliente / Unidade (1)
      slide150.getShapes()[24].getText().setText(CL2_MMR + " mil"); // MMR por Cliente / Unidade (2)
      slide150.getShapes()[25].getText().setText(CL3_MMR + " mil"); // MMR por Cliente / Unidade (3)
      slide150.getShapes()[26].getText().setText(CL4_MMR + " mil"); // MMR por Cliente / Unidade (4)
      slide150.getShapes()[27].getText().setText(CL5_MMR + " mil"); // MMR por Cliente / Unidade (5)

      // 💚💚💚💚💚💚💚💚💚💚💚💚💚💚💚💚💚💚💚 SLIDE 151 💜💜💜💜💜💜💜💜💜💜💜💜💜💜💜💜💜💜💜💜


        const slideIndex151 = 150; // Índice do slide (baseado em 0)

    // Acessar a apresentação e o slide
    var presentation = SlidesApp.openById(SLIDES_ID);
    var slide151 = presentation.getSlides()[slideIndex151];

    // Dados da planilha para o slide
    var DATAMOUNTH3 = sheet.getRange('C38').getValue();
    slide151.getShapes()[12].getText().setText(DATAMOUNTH3); // Período


    // Considerações finais
      var OBS_CONSID = sheet.getRange('B195').getValue();
      slide151.getShapes()[10].getText().setText(OBS_CONSID); // Box de dados a preencher - Considerações


    // Pontos de atenção
      var PONTOS_ATENC = sheet.getRange('B204').getValue();
      slide151.getShapes()[11].getText().setText(PONTOS_ATENC); // Box de dados a preencher - Pontos de atenção
    ...
    // Plano de ação - UNIDADES
      ...
      slide151.getShapes()[13].getText().setText(UNID_1); // Box de dados a preencher - UNIDADE 1
      slide151.getShapes()[14].getText().setText(UNID_2); // Box de dados a preencher - UNIDADE 2
      slide151.getShapes()[15].getText().setText(UNID_3); // Box de dados a preencher - UNIDADE 3
      slide151.getShapes()[16].getText().setText(UNID_4); // Box de dados a preencher - UNIDADE 4
      slide151.getShapes()[17].getText().setText(UNID_5); // Box de dados a preencher - UNIDADE 5
      ...
      slide151.getShapes()[8].getText().setText(PLAN_AC_UNID_1); // Box de dados a preencher - Plano de ação 1
      slide151.getShapes()[3].getText().setText(PLAN_AC_UNID_2); // Box de dados a preencher - Plano de ação 2
      slide151.getShapes()[18].getText().setText(PLAN_AC_UNID_3); // Box de dados a preencher - Plano de ação 3
      slide151.getShapes()[4].getText().setText(PLAN_AC_UNID_4); // Box de dados a preencher - Plano de ação 4
      slide151.getShapes()[5].getText().setText(PLAN_AC_UNID_5); // Box de dados a preencher - Plano de ação 5
